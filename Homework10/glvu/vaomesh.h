...
Eigen::Matrix4f orthogonality(float fovy, float aspect, float zNear, float zFar)
{
    //TODO
    assert(aspect > 0);
    assert(zFar > zNear);
    ...
    float tanHalfFovy = tan(radf / 2);
    Eigen::Matrix4f res = Eigen::Matrix4f::Zero(); 
    res(0, 0) = -1 / (aspect * tanHalfFovy * zNear);
    res(1, 1) = -1 / (tanHalfFovy * zNear);
...
struct GLMesh
{
    ...
    std::array<R, 16> matMVP(const int *vp, bool offsetDepth = false, bool colmajor = false) const {
        ...
        Mat4 proj = orthogonality(45.f, vp[2] / float(vp[3]), 3.5f, 100.0f);
